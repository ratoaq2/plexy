...
def path_references(text: str, prefixes: tuple[str, ...]) -> list[str]:
    """Return the repository paths that the text names in inline code. A pytest node id gives its file."""
    text = FENCED_BLOCK.sub("", text.replace("\r\n", "\n"))
    refs = (ref.partition("::")[0] for ref in INLINE_CODE.findall(text))
