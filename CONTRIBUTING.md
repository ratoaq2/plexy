...
- mypy: annotate every function. The mypy settings are in `pyproject.toml`.
- Ruff formats the code. Its settings are in `pyproject.toml`. Do not format by hand against it.
- Flat package layout: `plexy/`, not `src/plexy/`.
- No docstrings, except when the behavior is not clear from the code.
